...
**Q. API 키를 입력했는데 403 오류가 떠요**  
A. DeepL 계정에서 **API 키**와 **구독/계정 번호**를 혼동하는 경우가 많습니다.  
   올바른 API 키 찾는 방법:
   1. https://www.deepl.com/pro-api 접속 후 로그인
   2. 우측 상단 계정 → **Account** 클릭
   3. **API Keys** 섹션에서 +키 생성후 밑에 생성되는 키를 복사
   - 무료(Free) 키는 끝에 `:fx` 가 붙습니다 (예: `xxxx-xxxx-xxxx:fx`)
   - 구독 번호나 계정 ID가 아닌 **API Keys** 항목의 값이어야 합니다
   - 카드 미등록 시에도 403이 발생할 수 있습니다
