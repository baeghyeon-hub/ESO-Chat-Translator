...
![1.jpg](f105eb23-7b64-4e4e-8c9c-5738ebdc707c.png)

---

- 각 패널 이동 후

![2.jpg](7116ead1-6086-4a7b-853b-69c4e518edba.png)
...
- 모든 패널 자유 롭게 위치 조정, 크기 조정, 채팅 패널 투명화, 페이드 인,아웃 조절
...
- **Chat.log 자동 감지** — ESO 설치 경로 자동 탐색
...
1. [Releases](https://claude.ai/releases) 페이지에서 최신 버전 다운로드
2. 압축 풀기
...
- 툴 실행 후 상단 타이틀 바 오른쪽의 **⚙ 버튼**을 클릭합니다.
...
- `👁 표시` 버튼으로 입력 내용 확인 가능
- `🔌 API 연결 테스트` 버튼을 눌러 연결 확인
- 성공 시 `✅ 연결 성공` 및 잔여 사용량 표시

**② Chat.log 경로 설정**

- `🔍 자동 찾기` 버튼 클릭 → 자동으로 ESO 로그 파일 탐색
- 자동 감지 실패 시 `찾기` 버튼으로 직접 선택
- 기본 경로: `Documents/Elder Scrolls Online/live/Logs/ChatLog.log`
...
하단 패널의 **▶ 시작** 버튼을 클릭하면 Chat.log 감지가 시작됩니다.

ESO에서 채팅이 올라오면 자동으로 번역되어 표시됩니다.

![3.jpg](633b6e30-5cf9-4d33-93c0-b13196974d7d.png)

---

## 🖱 화면 구성 및 버튼 설명

![image.png](4cd8c14e-cf2c-4a3a-a6e7-45416b353851.png)
...
| 채널 패널 | 각 채널 버튼 | Say / Group / Zone / Guild / Whisper 필터 |
| 하단 패널 | `▶ 시작` / `⏹ 정지` | 채팅 감지 시작/정지 |
| 하단 패널 | `🗑 초기화` | 채팅 패널 내용 지우기 |
...
3. **한→영 입력창은 투과 모드에서도 그대로 사용 가능**
4. 다시 `🖱 투과` 클릭하면 해제

![4.jpg](7854159e-b9ee-4411-94bd-bbf493eb63d4.png)
...
A. `찾기` 버튼으로 직접 경로를 지정하세요.

경로: `C:\Users\사용자명\Documents\Elder Scrolls Online\live\Logs\ChatLog.log`
